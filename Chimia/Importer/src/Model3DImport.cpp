...
aiProcess_JoinIdenticalVertices);
  ...
  if (!scene)
  {
    printf("Model (%s) failed to load: %s", fileName,
importer.GetErrorString()); return;
  }
...
mesh->mVertices[i].y, mesh->mVertices[i].z }); if (mesh->mTextureCoords[0])
    {
...
indices.size()); meshList.push_back(newMesh);
...
void
Chimia::Importer::ImportModel(const std::string& path,
